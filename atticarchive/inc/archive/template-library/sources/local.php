...
use Archive\Core\Base\Document;
use Archive\Core\Editor\Editor;
use Archive\DB;
use Archive\Core\Settings\Manager as SettingsManager;
use Archive\Core\Settings\Page\Model;
use Archive\Modules\Library\Documents\Library_Document;
use Archive\Plugin;
...
/**
 * Archive template library local source.
 *
 * Archive template library local source handler class is responsible for
 * handling local Archive templates saved by the user locally on his site.
 *
 * @since 1.0.0
 */
class Source_Local extends Source_Base {
	...
	/**
	 * Archive template-library post-type slug.
	 */
	const CPT = 'Archive_library';

	/**
	 * Archive template-library taxonomy slug.
	 */
	const TAXONOMY_TYPE_SLUG = 'Archive_library_type';

	/**
	 * Archive template-library category slug.
	 */
	const TAXONOMY_CATEGORY_SLUG = 'Archive_library_category';

	/**
	 * Archive template-library meta key.
	 * @deprecated 2.3.0 Use \Archive\Core\Base\Document::TYPE_META_KEY instead
	 */
	const TYPE_META_KEY = '_Archive_template_type';

	/**
	 * Archive template-library temporary files folder.
	 */
	const TEMP_FILES_DIR = 'Archive/tmp';

	/**
	 * Archive template-library bulk export action name.
	 */
	const BULK_EXPORT_ACTION = 'Archive_export_multiple_templates';

	const ADMIN_MENU_SLUG = 'edit.php?post_type=Archive_library';

	const ADMIN_SCREEN_ID = 'edit-Archive_library';
	 ...
	 * other data.
	 *
	 * The local template class registers a new `Archive_library` post type
	 * and an `Archive_library_type` taxonomy. They are used to store data for
	...
	public function register_data() {
		$labels = [
			'name' => _x( 'My Templates', 'Template Library', 'Archive' ),
			'singular_name' => _x( 'Template', 'Template Library', 'Archive' ),
			'add_new' => _x( 'Add New', 'Template Library', 'Archive' ),
			'add_new_item' => _x( 'Add New Template', 'Template Library', 'Archive' ),
			'edit_item' => _x( 'Edit Template', 'Template Library', 'Archive' ),
			'new_item' => _x( 'New Template', 'Template Library', 'Archive' ),
			'all_items' => _x( 'All Templates', 'Template Library', 'Archive' ),
			'view_item' => _x( 'View Template', 'Template Library', 'Archive' ),
			'search_items' => _x( 'Search Template', 'Template Library', 'Archive' ),
			'not_found' => _x( 'No Templates found', 'Template Library', 'Archive' ),
			'not_found_in_trash' => _x( 'No Templates found in Trash', 'Template Library', 'Archive' ),
			'parent_item_colon' => '',
			'menu_name' => _x( 'Templates', 'Template Library', 'Archive' ),
		];
		...
		$args = [
			...
			'hierarchical' => false,
			'supports' => [ 'title', 'thumbnail', 'author', 'Archive' ],
		];

		/**
		 * Register template library post type args.
		 *
		 * Filters the post type arguments when registering Archive template library post type.
		 *
		 * @since 1.0.0
		 *
		 * @param array $args Arguments for registering a post type.
		 */
		$args = apply_filters( 'Archive/template_library/sources/local/register_post_type_args', $args );
		...
		$args = [
			...
			'rewrite' => false,
			'public' => false,
			'label' => _x( 'Type', 'Template Library', 'Archive' ),
		];

		/**
		 * Register template library taxonomy args.
		 *
		 * Filters the taxonomy arguments when registering Archive template library taxonomy.
		 *
		 * @since 1.0.0
		 *
		 * @param array $args Arguments for registering a taxonomy.
		 */
		$args = apply_filters( 'Archive/template_library/sources/local/register_taxonomy_args', $args );

		$cpts_to_associate = apply_filters( 'Archive/template_library/sources/local/register_taxonomy_cpts', [ self::CPT ] );
		...
		$args = [
			...
			'public' => false,
			'labels' => [
				'name' => _x( 'Categories', 'Template Library', 'Archive' ),
				'singular_name' => _x( 'Category', 'Template Library', 'Archive' ),
				'all_items' => _x( 'All Categories', 'Template Library', 'Archive' ),
			],
		];
		 ...
		 *
		 * Filters the category arguments when registering Archive template library category.
		 *
		 * @since 2.4.0
		 *
		 * @param array $args Arguments for registering a category.
		 */
		$args = apply_filters( 'Archive/template_library/sources/local/register_category_args', $args );
	...
	public function admin_menu() {
		add_submenu_page( self::ADMIN_MENU_SLUG, '', __( 'Saved Templates', 'Archive' ), Editor::EDITING_CAPABILITY, self::get_admin_url( true ) );
	}
	...
	public function save_item( $template_data ) {
		if ( ! current_user_can( $this->post_type_object->cap->edit_posts ) ) {
			return new \WP_Error( 'save_error', __( 'Access denied.', 'Archive' ) );
		}

		$defaults = [
			'title' => __( '(no title)', 'Archive' ),
			'page_settings' => [],
		 ...
		 *
		 * Fires after Archive template library was saved.
		 *
		 * @since 1.0.1
		 *
		 ...
		 */
		do_action( 'Archive/template-library/after_save_template', $template_id, $template_data );

		/**
		 * After template library update.
		 *
		 * Fires after Archive template library was updated.
		 *
		 * @since 1.0.1
		 *
		 * @param int   $template_id   The ID of the template.
		 * @param array $template_data The template data.
		 */
		do_action( 'Archive/template-library/after_update_template', $template_id, $template_data );
	...
	public function update_item( $new_data ) {
		...
		if ( ! $document ) {
			return new \WP_Error( 'save_error', __( 'Template not exist.', 'Archive' ) );
		}
		 ...
		 */
		do_action( 'Archive/template-library/after_update_template', $new_data['id'], $new_data );
	...
	public function get_item( $template_id ) {
		 ...
		 */
		$data = apply_filters( 'Archive/template-library/get_template', $data );
	...
	public function delete_template( $template_id ) {
		if ( ! current_user_can( $this->post_type_object->cap->delete_post, $template_id ) ) {
			return new \WP_Error( 'template_error', __( 'Access denied.', 'Archive' ) );
		}
	...
	public function export_multiple_templates( array $template_ids ) {
		...
		$zip_archive_filename = 'Archive-templates-' . gmdate( 'Y-m-d' ) . '.zip';
	 ...
	 *
	 * [
	 *  0 => '/www/wp-content/uploads/Archive/tmp/5eb3a7a411d44/templates/block-2-col-marble-title.json',
	 *  1 => '/www/wp-content/uploads/Archive/tmp/5eb3a7a411d44/templates/block-2-col-text-and-photo.json',
	 * ]
	 *
	 * @since 2.9.8
	 * @access private
	...
	public function import_template( $name, $path ) {
		...
		if ( 'zip' === $file_extension ) {
			...
			for ( $i = 0; $i < $zip->numFiles; $i++ ) {
				...
				// Template Kit zip files contain a `manifest.json` file, this is not a valid Archive template so ensure we skip it.
	...
	public function post_row_actions( $actions, \WP_Post $post ) {
		if ( self::is_base_templates_screen() ) {
			if ( $this->is_template_supports_export( $post->ID ) ) {
				$actions['export-template'] = sprintf( '<a href="%1$s">%2$s</a>', $this->get_export_link( $post->ID ), __( 'Export Template', 'Archive' ) );
			}
		}
	...
	public function admin_import_template_form() {
		...
		/** @var \Archive\Core\Common\Modules\Ajax\Module $ajax */
		$ajax = Plugin::$instance->common->get_component( 'ajax' );
		?>
		<div id="Archive-hidden-area">
			<a id="Archive-import-template-trigger" class="page-title-action"><?php echo __( 'Import Templates', 'Archive' ); ?></a>
			<div id="Archive-import-template-area">
				<div id="Archive-import-template-title"><?php echo __( 'Choose an Archive template JSON file or a .zip archive of Archive templates, and add them to the list of templates available in your library.', 'Archive' ); ?></div>
				<form id="Archive-import-template-form" method="post" action="<?php echo admin_url( 'admin-ajax.php' ); ?>" enctype="multipart/form-data">
					<input type="hidden" name="action" value="Archive_library_direct_actions">
					...
					<fieldset id="Archive-import-template-form-inputs">
						...
						<input type="submit" class="button" value="<?php echo esc_attr__( 'Import Now', 'Archive' ); ?>">
					</fieldset>
				</form>
			</div>
		</div>
	...
	public function is_template_supports_export( $template_id ) {
		 ...
		 */
		$export_support = apply_filters( 'Archive/template_library/is_template_supports_export', $export_support, $template_id );

		return $export_support;
	}

	/**
	 * Remove Archive post state.
	 *
	 * Remove the 'Archive' post state from the display states of the post.
	 *
	 * Used to remove the 'Archive' post state from the template library items.
	 *
	 ...
	 */
	public function remove_Archive_post_state_from_library( $post_states, $post ) {
		if ( self::CPT === $post->post_type && isset( $post_states['Archive'] ) ) {
			unset( $post_states['Archive'] );
		}
		return $post_states;
	}
	...
	private function get_export_link( $template_id ) {
		...
		return add_query_arg(
			[
				'action' => 'Archive_library_direct_actions',
				'library_action' => 'export_template',
				'source' => $this->get_id(),
				'_nonce' => wp_create_nonce( 'Archive_ajax' ),
	 ...
	 * library.
	 *
	 * Fired by `bulk_actions-edit-Archive_library` filter.
	 *
	 * @since 1.6.0
	 * @access public
	 *
	...
	public function admin_add_bulk_export_action( $actions ) {
		$actions[ self::BULK_EXPORT_ACTION ] = __( 'Export', 'Archive' );
	 ...
	 *
	 * Fired by `handle_bulk_actions-edit-Archive_library` filter.
	 *
	 * @since 1.6.0
	 * @access public
	 *
	 ...
	 *
	 * Fired by `views_edit-Archive_library` filter.
	 *
	 * @since 2.0.0
	 * @access public
	 *
	...
	public function maybe_render_blank_state( $which, array $args = [] ) {
		...
		$args = wp_parse_args( $args, [
			'cpt' => self::CPT,
			'post_type' => get_query_var( 'Archive_library_type' ),
		] );
		...
		if ( empty( $current_type ) ) {
			...
			$args['additional_inline_style'] = '#Archive-template-library-tabs-wrapper {display: none;}';
		}
	...
	private function render_blank_state( $current_type, array $args = [] ) {
		...
		$args = wp_parse_args( $args, [
			...
			'href' => '',
			'description' => __( 'Add templates and reuse them across your website. Easily export and import them to any other project, for an optimized workflow.', 'Archive' ),
		] );
		$inline_style .= $args['additional_inline_style'];
		?>
		<style type="text/css"><?php echo $inline_style; ?></style>
		<div class="Archive-template_library-blank_state">
	...
	public function print_blank_state_template( $current_type_label, $href, $description ) {
		?>
			<div class="Archive-blank_state">
				<i class="eicon-folder"></i>
				<h2>
					<?php
					/* translators: %s: Template type label. */
					printf( __( 'Create Your First %s', 'Archive' ), $current_type_label );
					?>
				</h2>
				<p><?php echo $description; ?></p>
				<a id="Archive-template-library-add-new" class="Archive-button Archive-button-success" href="<?php echo $href; ?>">
					<?php
					/* translators: %s: Template type label. */
					printf( __( 'Add New %s', 'Archive' ), $current_type_label );
					?>
				</a>
			</div>
		<?php
	...
	public function add_filter_by_category( $post_type ) {
		...
		echo '<label class="screen-reader-text" for="cat">' . _x( 'Filter by category', 'Template Library', 'Archive' ) . '</label>';
	...
	private function prepare_template_export( $template_id ) {
		...
		return [
			'name' => 'Archive-' . $template_id . '-' . gmdate( 'Y-m-d' ) . '.json',
	...
	private function get_template_label_by_type( $template_type ) {
		 ...
		 * @param string $template_type  Template type.
		 */
		$template_label = apply_filters( 'Archive/template-library/get_template_label_by_type', $template_label, $template_type );
	...
	private function add_actions() {
		if ( is_admin() ) {
			...
			add_filter( 'display_post_states', [ $this, 'remove_Archive_post_state_from_library' ], 11, 2 );
			...
			add_filter( 'bulk_actions-edit-Archive_library', [ $this, 'admin_add_bulk_export_action' ] );
			add_filter( 'handle_bulk_actions-edit-Archive_library', [ $this, 'admin_export_multiple_templates' ], 10, 3 );
	...
	public function admin_columns_content( $column_name, $post_id ) {
		if ( 'Archive_library_type' === $column_name ) {
	...
	public function admin_columns_headers( $posts_columns ) {
		...
		unset( $posts_columns['taxonomy-Archive_library_type'] );

		$offset = 2;

		$posts_columns = array_slice( $posts_columns, 0, $offset, true ) + [
			'Archive_library_type' => __( 'Type', 'Archive' ),
	...
	private function get_library_title() {
		...
		if ( $this->is_current_screen() ) {
			...
			if ( $current_tab_group ) {
				$titles = [
					'library' => __( 'Saved Templates', 'Archive' ),
					'theme' => __( 'Theme Builder', 'Archive' ),
					'popup' => __( 'Popups', 'Archive' ),
				];
