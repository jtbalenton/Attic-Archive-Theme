...
use Archive\Core\DynamicTags\Manager;
 ...
 * Archive database.
 *
 * Archive database handler class is responsible for communicating with the
 * DB, save and retrieve Archive data and meta data.
 *
 * @since 1.0.0
 */
class DB {
	 ...
	 * Is using Archive.
	 *
	 * Set whether the page is using Archive or not.
	 *
	 * @since 1.5.0
	 * @deprecated 3.1.0 Use `Plugin::$instance->documents->get( $post_id )->set_is_build_with_Archive( $is_Archive )` instead
	 * @access public
	 *
	 * @param int  $post_id      Post ID.
	 * @param bool $is_Archive Optional. Whether the page is Archive page.
	 *                           Default is true.
	 */
	public function set_is_Archive_page( $post_id, $is_Archive = true ) {
		Plugin::$instance->modules_manager
			->get_modules( 'dev-tools' )
			->deprecation
			->deprecated_function(
				__METHOD__,
				'3.1.0',
				'Plugin::$instance->documents->get( $post_id )->set_is_build_with_Archive( $is_Archive )'
			);
		...
		$document->set_is_built_with_Archive( $is_Archive );
	}
	 ...
	 * Iterate data.
	 *
	 * Accept any type of Archive data and a callback function. The callback
	 * function runs recursively for each element and his child elements.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 * @param array    $data_container Any type of Archive data.
	...
	/**
	 * Safely copy Archive meta.
	 *
	 * Make sure the original page was built with Archive and the post is not
	 * auto-save. Only then copy Archive meta from one post to another using
	 * `copy_Archive_meta()`.
	 *
	 * @since 1.9.2
	 * @access public
	 ...
	 */
	public function safe_copy_Archive_meta( $from_post_id, $to_post_id ) {
		// It's from  WP-Admin & not from Archive.
		if ( ! did_action( 'Archive/db/before_save' ) ) {
			$from_document = Plugin::$instance->documents->get( $from_post_id );

			if ( ! $from_document || ! $from_document->is_built_with_Archive() ) {
				return;
			}

			// It's an exited Archive auto-save
			if ( get_post_meta( $to_post_id, '_Archive_data', true ) ) {
				return;
			}
		}

		$this->copy_Archive_meta( $from_post_id, $to_post_id );
	}
	 ...
	 *
	 * Consider using `safe_copy_Archive_meta()` method instead.
	 *
	 * @since 1.1.0
	 * @access public
	 *
	 ...
	 * @param int $to_post_id   Target post ID.
	 */
	public function copy_Archive_meta( $from_post_id, $to_post_id ) {
		...
		foreach ( $from_post_meta as $meta_key => $values ) {
			// Copy only meta with the `_Archive` prefix
			if ( 0 === strpos( $meta_key, '_Archive' ) || in_array( $meta_key, $core_meta, true ) ) {
				$value = $values[0];

				// The Archive JSON needs slashes before saving
				if ( '_Archive_data' === $meta_key ) {
	...
	/**
	 * Is built with Archive.
	 *
	 * Check whether the post was built with Archive.
	 *
	 * @since 1.0.10
	 * @deprecated 3.2.0 Use `Plugin::$instance->documents->get( $post_id )->is_built_with_Archive()` instead
	 * @access public
	 *
	 * @param int $post_id Post ID.
	 *
	 * @return bool Whether the post was built with Archive.
	 */
	public function is_built_with_Archive( $post_id ) {
		Plugin::$instance->modules_manager
			->get_modules( 'dev-tools' )
			->deprecation
			->deprecated_function(
				__METHOD__,
				'3.2.0',
				'Plugin::$instance->documents->get( $post_id )->is_built_with_Archive()'
			);
		...
		return $document->is_built_with_Archive();
	}
	 ...
	 *
	 * Retrieve the post plain text from any given Archive data.
	 *
	 * @since 1.9.2
	 * @access public
	 *
