...
use Archive\Core\Settings\Base\CSS_Model;
use Archive\Plugin;
...
/**
 * Archive page settings model.
 *
 * Archive page settings model handler class is responsible for registering
 * and managing Archive page settings models.
 *
 * @since 1.6.0
 */
class Model extends CSS_Model {
	 ...
	 * Model constructor.
	 *
	 * Initializing Archive page settings model.
	 *
	 * @since 1.6.0
	 * @access public
	 *
	...
	public function get_panel_page_settings() {
		...
		return [
			...
			'title' => sprintf( __( '%s Settings', 'Archive' ), $document::get_title() ),
		];
	}
	 ...
	 * exclude it from the exported Archive data.
	 *
	 * @since 1.6.0
	 * @access public
	 *
	...
	public function on_export( $element_data ) {
		if ( ! empty( $element_data['settings']['template'] ) ) {
			/**
			 * @var \Archive\Modules\PageTemplates\Module $page_templates_module
			 */
			$page_templates_module = Plugin::$instance->modules_manager->get_modules( 'page-templates' );
			$is_Archive_template = ! ! $page_templates_module->get_template_path( $element_data['settings']['template'] );
