...
/**
 * Archive settings base manager.
 *
 * Archive settings base manager handler class is responsible for registering
 * and managing Archive settings base managers.
 *
 * @since 1.6.0
 * @abstract
 */
abstract class Manager {
	 ...
	 *
	 * Initializing Archive settings base manager.
	 *
	 * @since 1.6.0
	 * @access public
	 */
	public function __construct() {
		add_action( 'Archive/editor/init', [ $this, 'on_Archive_editor_init' ] );

		add_action( 'Archive/ajax/register_actions', [ $this, 'register_ajax_actions' ] );
	}
	 ...
	 *
	 * Fired by `Archive/ajax/register_actions` action.
	 *
	 * @since 2.0.0
	 * @access public
	 *
	...
	final public function ajax_save_settings( $request ) {
		 ...
		 */
		$success_response_data = apply_filters( "Archive/settings/{$settings_name}/success_response_data", $success_response_data, $id, $data );
	 ...
	 *
	 * Fired by `Archive/init` action.
	 *
	 * @since 2.3.0
	 * @access public
	 */
	public function on_Archive_editor_init() {
	...
	protected function print_editor_template_content( $name ) {
		?>
		<#
		const tabs = Archive.config.settings.<?php echo $name; ?>.tabs;

		if ( Object.values( tabs ).length > 1 ) { #>
			<div class="Archive-panel-navigation">
				<# _.each( tabs, function( tabTitle, tabSlug ) {
					$e.bc.ensureTab( 'panel/<?php echo $name; ?>-settings', tabSlug ); #>
					<div class="Archive-component-tab Archive-panel-navigation-tab Archive-tab-control-{{ tabSlug }}" data-tab="{{ tabSlug }}">
						<a href="#">{{{ tabTitle }}}</a>
					</div>
				<# } ); #>
			</div>
		<# } #>
		<div id="Archive-panel-<?php echo $name; ?>-settings-controls"></div>
		<?php
	}
	...
	private function get_editor_template() {
		...
		ob_start();
		?>
		<script type="text/template" id="tmpl-Archive-panel-<?php echo esc_attr( $name ); ?>-settings">
