...
class ActionPost extends ActionBase {
		...
		$widget->add_control(
			...
			[
				...
				'options' => [
					...
					//'approval_and_verify' => __( 'Approval and Email Verification', 'acf-frontend-form-element' ),
				],
			]
		);
	...
	public function run( $form, $step = false ){	
		...
		$form = $this->save_post( $form, $post_to_edit, $metas, $element );
		return $form;
	}

	public function save_post( $form, $post_to_edit, $metas, $element ){
