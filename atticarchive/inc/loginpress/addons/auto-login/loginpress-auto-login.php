...
if ( ! class_exists( 'LoginPress_AutoLogin' ) ) :
	...
	final class LoginPress_AutoLogin {
      ...
      add_action( 'admin_init', array( $this, 'init_addon_updater' ), 0 );
		...
		private function define_constants() {
			...
			$this->define( 'LOGINPRESS_AutoLogin_PLUGIN_BASENAME', plugin_basename( LOGINPRESS_ROOT_FILE ) );
			$this->define( 'LOGINPRESS_AutoLogin_DIR_PATH', plugin_dir_path( LOGINPRESS_ROOT_FILE ) );
			$this->define( 'LOGINPRESS_AutoLogin_DIR_URL', plugin_dir_url( LOGINPRESS_ROOT_FILE ) );
			$this->define( 'LOGINPRESS_AutoLogin_ROOT_PATH',  dirname( LOGINPRESS_ROOT_FILE ) . '/' );
			$this->define( 'LOGINPRESS_AutoLogin_ROOT_FILE', LOGINPRESS_ROOT_FILE );
			$this->define( 'LOGINPRESS_AUTOLOGIN_VERSION', $this->version );
			$this->define( 'LOGINPRESS_AUTOLOGIN_PLUGIN_ROOT', dirname( plugin_basename( LOGINPRESS_ROOT_FILE ) ) );
		}
    ...
    public function init_addon_updater() {
        if( class_exists( 'LoginPress_AddOn_Updater' ) ) {
          //echo 'Exists';
          $updater = new LoginPress_AddOn_Updater( 2324, LOGINPRESS_ROOT_FILE, $this->version );
        }
      }
		...
		function _admin_scripts( $hook ) {
			...
			wp_enqueue_style( 'loginpress_autologin_stlye', my_plugin_ur( 'css/style.css', LOGINPRESS_ROOT_FILE ), array(), LOGINPRESS_AUTOLOGIN_VERSION );

			wp_enqueue_script( 'loginpress_autologin_js', my_plugin_ur( 'js/autologin.js', LOGINPRESS_ROOT_FILE ), array(), LOGINPRESS_AUTOLOGIN_VERSION );
