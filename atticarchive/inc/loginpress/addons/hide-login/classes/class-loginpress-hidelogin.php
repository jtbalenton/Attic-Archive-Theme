		...
		*/
		...
		function loginpress_hidelogin_admin_action_scripts( $hook ) {
			...
			if ( "toplevel_page_loginpress-settings" == $hook ) {
				...
				wp_register_style( 'loginpress-admin-hidelogin', my_plugin_ur( 'assets/css/style.css', dirname( LOGINPRESS_ROOT_FILE ) ), array(), LOGINPRESS_HIDE_VERSION );
				wp_enqueue_style( 'loginpress-admin-hidelogin' );
			}

			wp_enqueue_script( 'loginpress-admin-hidelogin', my_plugin_ur( 'assets/js/required-action.js', dirname( LOGINPRESS_ROOT_FILE ) ), array( 'jquery' ), LOGINPRESS_HIDE_VERSION );
