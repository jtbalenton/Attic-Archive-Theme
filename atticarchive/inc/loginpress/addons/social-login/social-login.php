  ...
  final class LoginPress_Social {
    ...
    */
    private function define_constants() {
      ...
      $this->define( 'LOGINPRESS_SOCIAL_PLUGIN_BASENAME', plugin_basename( LOGINPRESS_ROOT_FILE ) );
      $this->define( 'LOGINPRESS_SOCIAL_DIR_PATH', plugin_dir_path( LOGINPRESS_ROOT_FILE ) );
      $this->define( 'LOGINPRESS_SOCIAL_DIR_URL', plugin_dir_url( LOGINPRESS_ROOT_FILE ) );
      $this->define( 'LOGINPRESS_SOCIAL_ROOT_PATH', dirname( LOGINPRESS_ROOT_FILE ) . '/' );
    ...
    */
    private function _hooks() {
      ...
      add_action( 'wp_loaded', array( $this, 'textdomain' ), 30 );
    ...
    */
    public function init_addon_updater() {
      if ( class_exists( 'LoginPress_AddOn_Updater' ) ) {
        // echo 'Exists';
        $updater = new LoginPress_AddOn_Updater( 2335, LOGINPRESS_ROOT_FILE, $this->version );
      }
    }
    ...
    function loginpress_social_login_admin_action_scripts( $hook ) {
      if ( 'toplevel_page_loginpress-settings' == $hook ) {
        wp_enqueue_style( 'loginpress-admin-social-login', my_plugin_ur( 'assets/css/style.css', LOGINPRESS_ROOT_FILE ), array(), LOGINPRESS_SOCIAL_VERSION );
        wp_enqueue_script( 'loginpress-admin-social-login', my_plugin_ur( 'assets/js/main.js', LOGINPRESS_ROOT_FILE ), false, LOGINPRESS_SOCIAL_VERSION );
      }
    }
    ...
    function loginpress_social_login_scripts( $posts ) {
        ...
        if ( $allow ) {
            ...
            wp_enqueue_style( 'loginpress-social-login', my_plugin_ur( 'assets/css/login.css', LOGINPRESS_ROOT_FILE ), array(), LOGINPRESS_SOCIAL_VERSION );
        }
        return $posts;
    }
...
register_activation_hook( LOGINPRESS_ROOT_FILE, array( 'LoginPress_Social', 'loginpress_social_activation' ) );
add_action( 'wpmu_new_blog', array( 'LoginPress_Social', 'loginpress_social_activation' ) );

add_action( 'wp_loaded', 'lp_sl_instance', 25 );
