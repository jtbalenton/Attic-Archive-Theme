	...
	final class LoginPress_Login_Widget {
      ...
      $this->define_constants();
		...
		public function _hooks() {
			...
			// Here we call `init` action instead of `wp_loaded` for textdomain(); because wp_loaded is triggered before the theme loads.
      ...
      $encoded_url = urlencode( $redirect_to );
	    ...
	    public function init_addon_updater() {
	        if( class_exists( 'LoginPress_AddOn_Updater' ) ) {
	          //echo 'Exists';
	          $updater = new LoginPress_AddOn_Updater( 2333, LOGINPRESS_ROOT_FILE, $this->version );
	        }
	    }
		...
		public function textdomain() {
			...
			$plugin_dir =  dirname( plugin_basename( LOGINPRESS_ROOT_FILE ) ) ;
    ...
    function social_login() {
      if ( class_exists( 'LoginPress_Social' ) && true === apply_filters( 'loginpress_social_widget', true ) ) {
		...
		public function _widget_script() {
			...
			wp_enqueue_script( 'loginpress-login-widget-script', my_plugin_ur( 'assets/js/script.js', LOGINPRESS_ROOT_FILE ), array( 'jquery' ), $this->version, false );

			// Enqueue Styles
			wp_enqueue_style( 'loginpress-login-widget-style', my_plugin_ur( 'assets/css/style.css', LOGINPRESS_ROOT_FILE ), '', $this->version );
      ...
      if ( class_exists( 'LoginPress_Social' ) ) {
        ...
        wp_enqueue_style( 'loginpress-social-login', my_plugin_ur( 'loginpress-social-login/assets/css/login.css', __DIR__ ), array(), LOGINPRESS_SOCIAL_VERSION );
      }
		...
		private function define_constants() {
			...
			$this->define( 'LOGINPRESS_WIDGET_PLUGIN_BASENAME', plugin_basename( LOGINPRESS_ROOT_FILE ) );
			$this->define( 'LOGINPRESS_WIDGET_DIR_PATH', plugin_dir_path( LOGINPRESS_ROOT_FILE ) );
			$this->define( 'LOGINPRESS_WIDGET_DIR_URL', plugin_dir_url( LOGINPRESS_ROOT_FILE ) );
			$this->define( 'LOGINPRESS_WIDGET_ROOT_PATH',  dirname( LOGINPRESS_ROOT_FILE ) . '/' );
			$this->define( 'LOGINPRESS_WIDGET_ROOT_FILE', LOGINPRESS_ROOT_FILE );
		...
		function _admin_scripts( $hook ) {
			...
			wp_enqueue_style( 'loginpress_widget_stlye', my_plugin_ur( 'assets/css/style.css', LOGINPRESS_ROOT_FILE ), array(), LOGINPRESS_WIDGET_VERSION );

			wp_enqueue_script( 'loginpress_widget_js', my_plugin_ur( 'assets/js/script.js', LOGINPRESS_ROOT_FILE ), array( 'jquery' ), LOGINPRESS_WIDGET_VERSION );
...
add_action( 'wp_loaded', 'lp_lw_instance', 25 );
